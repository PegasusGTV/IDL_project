...
class ASRTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            feats, targets_shifted, targets_golden = feats.to(self.device, dtype=torch.float16), targets_shifted.to(self.device), targets_golden.to(self.device)
            feat_lengths, transcript_lengths = feat_lengths.to(self.device), transcript_lengths.to(self.device)
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                if recognition_config['beam_width'] > 1:
                    ...
                    seqs, scores = generator.beam_search(prompts, recognition_config['beam_width'], recognition_config['temperature'], recognition_config['repeat_penalty'])
