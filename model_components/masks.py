...
def PadMask(padded_input, input_lengths):
    ...
    range_tensor = torch.arange(T, device=padded_input.device).expand(len(input_lengths), T)
    mask = range_tensor >= input_lengths[:, None]
    return mask
...
def CausalMask(padded_input):
    ...
    row_indices = torch.arange(T, device=padded_input.device).view(-1, 1)
    col_indices = torch.arange(T, device=padded_input.device).view(1, -1)
